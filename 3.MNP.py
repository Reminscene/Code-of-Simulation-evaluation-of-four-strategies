...
def profit_net(current_grid_id, next_grid_id):
    net_profit_exp_1 = 0
    net_profit_exp_2 = 0
    matching_prob = Matching_prob_df.loc[next_grid_id, 'matching_prob']
    d_dis = Grid_OD_distance_mtx[current_grid_id][next_grid_id]
    df_neighbor_next = copy.deepcopy(Neighbour_df.groupby('current_grid_id').get_group(next_grid_id))
    df_neighbor_next.reset_index(inplace=True)
    neighbor_next_lst = list(pd.unique(df_neighbor_next['neighbor_grid_id']))
    for i in range(0, len(Destination_prob_mtx[next_grid_id])): # i是乘客的目的地网格编号
        destination_prob = list(Destination_prob_mtx[next_grid_id])[i]
        if destination_prob == 0:
            pass
        else:
            c_dis = Grid_OD_distance_mtx[next_grid_id][i]
            net_profit_exp_1 += destination_prob * (fee(c_dis) - c_dis*cost_alpha)
    for j in neighbor_next_lst:
        if len(neighbor_next_lst) != 0:
            destination_prob = 1/(len(neighbor_next_lst))
        else:
            destination_prob = 0
        v_dis = Grid_OD_distance_mtx[next_grid_id][j]
        net_profit_exp_2 += destination_prob * (0 - v_dis*cost_alpha)
    v_dis = Grid_OD_distance_mtx[next_grid_id][next_grid_id]
    net_profit_exp_2 += (0 - v_dis * cost_alpha)
    net_profit_exp = matching_prob*net_profit_exp_1 + (1-matching_prob)*net_profit_exp_2 - d_dis*cost_alpha
    return net_profit_exp
...
def simulation(grid_id, end_distance, vehicle_num):  # Output the average revenue and rate of vacant of the vehicle.
    ...
    while n < vehicle_num:
        while distance < end_distance:
            ...
            matching_state = random_index([no_matching_prob, matching_prob])
            ...
            net_profit_lst = []
            for l in neighbor_lst:
                profit = profit_net(current_grid_id,l)
                net_profit_lst.append(profit)
            max_profit_grid_index = net_profit_lst.index(max(net_profit_lst))
            max_profit_grid_id = neighbor_lst[max_profit_grid_index]

            if matching_state == 0:  # The next location is the grid corresponding to the action.
                next_grid_id = max_profit_grid_id
...
file_save = open('C://Users//张晨皓//Desktop//Simulation evaluation//result//0-1MNP.txt', 'w').close()
file_save_handle = open('C://Users//张晨皓//Desktop//Simulation evaluation//result//0-1MNP.txt', mode='a')
